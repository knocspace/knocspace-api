...
6개 중 3개는 확정했고, 3개(`If-Match` 헤더, `position`, 시드 유저)는 아래 이유로 아직 보류입니다.
...
### 3. `position` 은 소수를 계속 끼워넣을 수 없습니다 — 보류

두 형제 사이 중간값을 반복해서 넣으면 `float64`는 **50번 안쪽에서 두 값이 같아집니다.** 트리 드래그 이동(F5)에서 실제로 발생합니다.

**CRDT를 먼저 공부하고 결정합니다.** 노션류 협업 툴이 쓰는 문자열 fractional index는 동시 편집(B8) 상황을 겨냥한 해법이라, 지금 필요한지부터 판단이 필요합니다.

결정 전까지의 잠정안(코드에는 아직 반영 안 함):
...
### 5. 로그인이 B4인데 `createdBy`는 처음부터 있습니다 — 보류

**Prisma 스키마(`Workspace`/`User`/`Membership`)를 확정한 뒤 결정합니다.** 잠정 방향은 B1 마이그레이션에서 워크스페이스 1개·사용자 1명(`local-user`)을 시드로 넣고 B4에서 실제 계정으로 승격시키는 것이지만, 스키마 확정 전엔 코드로 옮기지 않습니다.
